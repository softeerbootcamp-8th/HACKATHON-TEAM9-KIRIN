...
>
> **서비스명은 "오다가다"** 다(로고 워드마크·브라우저 탭 타이틀 등 사용자 노출 문구).
> `KIRIN` 은 리포지토리·패키지명 등에 남아 있는 내부 프로젝트 코드네임이다.
...
│  └─ index.tsx          # "/" — 화면이 늘어나면 규칙에 맞게 파일을 추가한다
├─ api/
...
├─ lib/
│  ├─ utils.ts           # cn() 등 범용 유틸
│  └─ query-client.ts    # TanStack Query 전역 클라이언트
└─ styles/
...
- 파일 규약: `index.tsx` → `/`, `login.tsx` → `/login`,
  `foo/$id.tsx` → `/foo/:id`, `_layout.tsx` → 레이아웃 라우트(pathless).
...
2. 생성된 **TanStack Query 훅** 만 사용한다. axios 직접 호출 금지.
...
- 실시간 갱신이 필요한 화면은 개별 쿼리에서 `refetchInterval`/폴링 또는
