    ...
    public void notifyError(RuntimeException exception, ErrorRequestContext context) {
        ...
        try {
            Map<String, Object> payload =
                    SlackErrorMessageFactory.buildPayload(exception, context, activeProfile, slackProperties.channel());
            ...
            if (response == null || !response.ok()) {
                ...
                log.warn("Slack 에러 알림 전송이 거절됨 - channel={}, error={}", slackProperties.channel(), error);
            }
        } catch (Exception sendFailure) {
            log.error("Slack 에러 알림 전송 실패 - channel={}", slackProperties.channel(), sendFailure);
        }
    }
}
