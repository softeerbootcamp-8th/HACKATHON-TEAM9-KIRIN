...
| 사용상태(UsageStatus) | 물품보관함의 사용 진행 상태. `AVAILABLE`(비어 있음) / `RESERVED`(예약됨) / `OCCUPIED`(물품 보관 중) 세 값을 가진다. 잠금상태와 별개다. |
| 물품(Product) | 판매자가 등록하고 사물함을 예약해 판매하는 중고 물품 1건. |
| 물품상태(ProductStatus) | 물품의 판매 진행 상태. `PREPARING`(등록됨, 사물함 예약 전) / `RESERVED`(사물함 예약됨) / `SELLING`(판매중) / `SOLD`(판매완료) / `EXPIRED`(판매기간 만료, 회수 대기) 다섯 값을 가진다. |
| 사물함 예약(LockerReservation) | 판매자가 물품 1건을 지정해 비어 있는 물품보관함을 4시간 동안 선점하는 행위. 예약은 물품의 활성 상태와 시간 정보로 관리한다. |
| 물품 투입(Deposit) | 예약한 물품보관함의 문을 열고 물품을 넣은 뒤 잠가 판매를 시작하는 행위. |
| 즉시판매(SellImmediately) | "바로 팔기"로 물품보관함을 예약함과 동시에, 문을 잠그는 절차 없이 곧바로 판매중(SELLING) 상태로 전환하는 행위. 데모용으로, 정식 물품 투입 절차(투입 시작·완료 확인)를 생략한다. |
| 판매자 회수(Recovery) | 판매기간이 만료됐거나 판매자가 판매를 조기 종료한 물품을 판매자가 물품보관함에서 꺼내는 행위. 조기 종료 시 물품상태는 즉시 판매기간 만료(EXPIRED)로 전환되고, 이후 흐름은 자연 만료와 동일하다. |
| 판매자명(sellerName) | 물품을 등록한 회원의 표시용 이름. 등록 시점 회원의 닉네임(nickname)을 그대로 복사해 저장하며, 실제 소유자 판별은 `sellerMemberId`(회원 ID)로 한다. |
...
| 구매자명(buyerName) | 물품을 구매한 회원의 표시용 이름. 구매 시점 회원의 닉네임(nickname)을 그대로 복사해 저장하며, 실제 소유자 판별은 `buyerMemberId`(회원 ID)로 한다. |
