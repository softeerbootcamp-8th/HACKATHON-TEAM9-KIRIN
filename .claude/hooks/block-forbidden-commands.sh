#!/usr/bin/env bash
# backend/CLAUDE.md "위험 명령어" 목록의 파괴적 명령을 PreToolUse 단계에서 차단한다.
...
check_sql_segment() {
  ...
  printf '%s' "$seg" | grep -Eiq "${SQLQ}drop[[:space:]]+(table|database|schema|index|view|user|role)" &&
    deny "위험 명령: DROP. DB schema 변경은 팀 확인 후 사람이 직접 실행한다."

  printf '%s' "$seg" | grep -Eiq "${SQLQ}truncate[[:space:]]+(table|[[:alnum:]_\`]+)" &&
    deny "위험 명령: TRUNCATE. 데이터 삭제는 팀 확인 후 사람이 직접 실행한다."
  return 0
}
